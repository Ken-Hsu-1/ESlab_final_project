...
int8_t SPI_WIFI_Init(uint16_t mode)
{
  ...
  if (mode == ES_WIFI_INIT)
  {
    ...
    spi_rx_sem = osSemaphoreNew(1, 1, osSemaphore(spi_rx_sem));
    spi_tx_sem = osSemaphoreNew(1, 1, osSemaphore(spi_tx_sem));
    cmddata_rdy_rising_sem = osSemaphoreNew(1, 1, osSemaphore(cmddata_rdy_rising_sem));
    /* take semaphore */
