...
class GestureSyncMusicController:
    ...
    def process_gesture_command(self, command):
        ...
        try:
            ...
            if self.volume_changing and self.volume_control_mode == 'absolute':
                try:
                    # Convert received rotation to volume level
                    volume_level = self.map_rotation_to_volume(float(command))
                    self.set_absolute_volume(volume_level)
                    return
                except ValueError:
    ...
    def map_rotation_to_volume(self, rotation):
        ...
        Assumes rotation range of 0-360 degrees
        """
